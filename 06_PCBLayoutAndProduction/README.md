# Physical Computing

## PCB-Layout & Production
<img src="https://www.hackteria.org/wiki/images/5/5d/IMG_20170408_142207.jpg" width="540" title="CC BY-SA 4.0 @tamberg" alt="A custom temperature sensor breakout board"/><br/>
_Gär-Lämpli, a custom temperature sensor breakout board drawn by participants in Hammamatsu, Japan_

### Contents
- [Motivation](#Motivation)
- [Basics](#Basics)
- [Advanced](#Advanced)
- [Materials](#Materials)
- [Tools](#Tools)
- [Skills](#Skills)
- [Resources](#Resources)

### Motivation
- [MicroNoise SZ-ready](https://github.com/sgmk/microNoise_SZ-Ready) on GitHub or [Kitspace](https://www.kitspace.org/kit-index/micronoise-sz-ready/), by SGMK since 2008
- [UROŠ CO2 breakout](https://github.com/GenericLab/UROS-CO2_brkout) on GitHub, by @dusjagr
- [8-bit mixtape project](https://github.com/8BitMixtape/8Bit-Mixtape-NEO) on GitHub, by @dusjagr et al.
- [Amazing creative PCB designs](https://darcyneal.com) website, by Darcy Neal
- [#MakeZurich Badge 2023](http://www.tamberg.org/makezurich/2023/MakeZurichBadge2023.pdf) slides, by @tamberg

### Basics
- Placing components
- Drawing conenctions
- Preventing crossings
- ...

### Advanced
- Creating a BOM
- Sourcing parts
- Adding a footprint
- ...

### Materials
- Paper
- Tape
- ...

### Tools
- https://www.hackteria.org/wiki/Diy-CAD
- https://github.com/badgeek/svg2shenzhen
- https://github.com/wntrblm/Gingerbread
- Layout Software, e.g. Inkscape, KiCad or Fritzing
- Your computer
- Scissors
- Pencil
- ...

### Skills
- [ ] SMD soldering
- [ ] Layout a circuit by hand
- [ ] Inkscape PCB Layout preparation svg2shenzhen
- [ ] Edit a circuit in KiCad
- [ ] Order a PCB online
- [ ] ...

### Resources
- [SMD soldering](SMDSoldering/README.md) tutoria, by @dusjagr










